(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["default~BloodMoon~index"],{

/***/ "7gfb":
/*!*****************************************************************!*\
  !*** ./node_modules/bcdice/lib/bcdice/game_system/BloodMoon.js ***!
  \*****************************************************************/
...
Opal.modules["bcdice/game_system/BloodCrusade"] = function (Opal) {
  ...
  return function ($base, $parent_nesting) {
    ...
    (function ($base, $parent_nesting) {
      ...
      (function ($base, $super, $parent_nesting) {
        ...
        var $nesting = [self].concat($parent_nesting),
            $BloodMoon_initialize$1,
            $BloodMoon_result_2d6$2,
            $BloodMoon_eval_game_system_specific_command$3;
        Opal.const_set($nesting[0], 'ID', "BloodMoon");
        Opal.const_set($nesting[0], 'NAME', "ブラッドムーン");
        Opal.const_set($nesting[0], 'SORT_KEY', "ふらつとむうん");
        Opal.const_set($nesting[0], 'HELP_MESSAGE', "" + "・各種表\n" + "　・関係属性表　RAT\n" + "　・導入タイプ決定表(ノーマル)　IDT\n" + "　・導入タイプ決定表(ハード込み)　ID2T\n" + "　・シーン表           ST\n" + "　・先制判定指定特技表 IST\n" + "　・身体部位決定表　　 BRT\n" + "　・自信幸福表　　　　 CHT\n" + "　・地位幸福表　　　　 SHT\n" + "　・日常幸福表　　　　 DHT\n" + "　・人脈幸福表　　　　 LHT\n" + "　・退路幸福表　　　　 EHT\n" + "　・ランダム全特技表　 AST\n" + "　・軽度狂気表　　　　 MIT\n" + "　・重度狂気表　　　　 SIT\n" + "・D66ダイスあり\n");
        Opal.def(self, '$initialize', $BloodMoon_initialize$1 = function $$initialize(command) {
          var $iter = $BloodMoon_initialize$1.$$p,
              $yield = $iter || nil,
              self = this;
          if ($iter) $BloodMoon_initialize$1.$$p = null;
          $send2(self, Opal.find_super_dispatcher(self, 'initialize', $BloodMoon_initialize$1, false, true), 'initialize', [command], null);
          self.sort_add_dice = true;
          self.d66_sort_type = $$$($$($nesting, 'D66SortType'), 'ASC');
          return self.round_type = $$$($$($nesting, 'RoundType'), 'CEIL');
        }, $BloodMoon_initialize$1.$$arity = 1);
        Opal.def(self, '$result_2d6', $BloodMoon_result_2d6$2 = function $$result_2d6(total, dice_total, _dice_list, cmp_op, target) {
          var self = this;
          ...
          if ($truthy($rb_le(dice_total, 2))) {
            return $$($nesting, 'Result').$fumble("ファンブル(【余裕】が 0 に)");
          } else if ($truthy($rb_ge(dice_total, 12))) {
            return $$($nesting, 'Result').$critical("スペシャル(【余裕】+3）");
          } else if (target['$==']("?")) {
          ...
          ;
        }, $BloodMoon_result_2d6$2.$$arity = 5);
        Opal.def(self, '$eval_game_system_specific_command', $BloodMoon_eval_game_system_specific_command$3 = function $$eval_game_system_specific_command(command) {
          var self = this,
              $ret_or_1 = nil;
          return function () {
            if ($truthy($ret_or_1 = self.$roll_tables(command, $$($nesting, 'TABLES')))) {
              ...
              return $$$($$($nesting, 'BloodCrusade'), 'RTT').$roll_command(self.$randomizer(), command);
            }

            ;
            return nil;
          }();
        }, $BloodMoon_eval_game_system_specific_command$3.$$arity = 1);
        Opal.const_set($nesting[0], 'TABLES', $hash2(["ST", "MIT", "SIT", "CHT", "SHT", "DHT", "LHT", "EHT", "ID2T", "IDT", "RAT"], {
          "ST": $$$($$($nesting, 'DiceTable'), 'Table').$new("シーン表", "2D6", ["どこまでも広がる荒野。風が吹き抜けていく。", "血まみれの惨劇の跡。いったい誰がこんなことを？", "都市の地下。かぼそい明かりがコンクリートを照らす。", "豪華な調度が揃えられた室内。くつろぎの空間を演出。", "普通の道端。様々な人が道を行き交う。", "明るく浮かぶ月の下。暴力の気配が満ちていく。", "打ち捨てられた廃墟。荒れ果てた景色に心も荒む。", "生活の様子が色濃く残る部屋の中。誰の部屋だろう？", "にぎやかな飲食店。騒ぐ人々に紛れつつ事態は進行する。", "ざわめく木立。踊る影。", "高い塔の上。都市を一望できる。"]),
          "MIT": $$$($$($nesting, 'DiceTable'), 'Table').$new("軽度狂気表", "1D6", ["【誇大妄想】（判定に失敗するたびに【テンション】が１増加する。）", "【記憶喪失】（【幸福】の修復判定にマイナス２の修正。）", "【こだわり】（戦闘中の行動を「パス」以外で一つ選択し、その行動をすると【テンション】が６増加する。）", "【お守り中毒】（「幸運のお守り」を装備していない場合、全ての2d6判定にマイナス１の修正。）", "【不死幻想】（自分が受けるダメージが全て１増加する。）", "【血の飢え】（戦闘中、ラウンドごとに他のキャラクターにダメージを与えないと、ラウンド終了時に【テンション】４増加。）"]),
          "SIT": $$$($$($nesting, 'DiceTable'), 'Table').$new("重度狂気表", "1D6", ["【幸福依存】（【幸福】を一つ選択し、その【幸福】が結果フェイズに失われたとき、死亡する。）", "【見えない友達】（交流判定にマイナス３の修正がつく。）", "【臆病】（自分の行う妨害判定にマイナス２の修正がつく。）", "【陰謀論】（休息判定にマイナス３の修正がつく。）", "【指令受信】（メインフェイズの３サイクル目の自分のシーンでは、可能な範囲でGMが行動を決定する。）", "【猜疑心】（自分が「連携攻撃」を行うとき、関係の【深度】をダメージに加えられない。）"]),
...
//# sourceMappingURL=default~BloodMoon~index.js.map