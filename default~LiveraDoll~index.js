(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["default~LiveraDoll~index"],{

/***/ "zi63":
/*!******************************************************************!*\
  !*** ./node_modules/bcdice/lib/bcdice/game_system/LiveraDoll.js ***!
  \******************************************************************/
  ...
  return function ($base, $parent_nesting) {
    ...
    (function ($base, $parent_nesting) {
      ...
      (function ($base, $super, $parent_nesting) {
        ...
        var $nesting = [self].concat($parent_nesting),
            $LiveraDoll_initialize$1,
            $LiveraDoll_eval_game_system_specific_command$2,
            $LiveraDoll_checkRoll$3,
            $LiveraDoll_get_card_text$5;
        ...
        self.$register_prefix("(C|K|W|R|B|G|E)(L|D|O)\\d+", "(\\d+)?ATK([1-6])?([1-6])?([1-6])?([1-6])?([1-6])?([1-6])?");
        Opal.def(self, '$initialize', $LiveraDoll_initialize$1 = function $$initialize(command) {
          var $iter = $LiveraDoll_initialize$1.$$p,
              $yield = $iter || nil,
              self = this;
          if ($iter) $LiveraDoll_initialize$1.$$p = null;
          $send2(self, Opal.find_super_dispatcher(self, 'initialize', $LiveraDoll_initialize$1, false, true), 'initialize', [command], null);
          self.sort_add_dice = true;
          return self.sort_barabara_dice = true;
        }, $LiveraDoll_initialize$1.$$arity = 1);
        Opal.def(self, '$eval_game_system_specific_command', $LiveraDoll_eval_game_system_specific_command$2 = function $$eval_game_system_specific_command(command) {
          var self = this,
              $case = nil,
              diceCount = nil,
              $ret_or_1 = nil,
          ...
          return function () {
            ...
            if (/^(\d+)?ATK([1-6])?([1-6])?([1-6])?([1-6])?([1-6])?([1-6])?$/i['$===']($case)) {
              ...
              blockNo = [function () {
                ...
                if ($truthy($ret_or_4 = $$($nesting, 'Regexp').$last_match(4))) {
                  return $ret_or_4;
                } else {
                  return 0;
                }
                ...
                if ($truthy($ret_or_5 = $$($nesting, 'Regexp').$last_match(5))) {
                  return $ret_or_5;
                } else {
                  return 0;
                }
                ...
                if ($truthy($ret_or_6 = $$($nesting, 'Regexp').$last_match(6))) {
                  return $ret_or_6;
                } else {
                  return 0;
                }
                ...
                if ($truthy($ret_or_7 = $$($nesting, 'Regexp').$last_match(7))) {
                  return $ret_or_7;
                } else {
                  return 0;
                }
              ...
              return self.$checkRoll(diceCount, blockNo);
        ...
        }, $LiveraDoll_eval_game_system_specific_command$2.$$arity = 1);
        ...
        return (Opal.def(self, '$get_card_text', $LiveraDoll_get_card_text$5 = function $$get_card_text(color, cardtype, cardindex) {
          var self = this,
              $case = nil,
              table = nil;
          ...
          if ("C"['$===']($case)) {
            ...
            if ("L"['$===']($case)) {
              table = [[1, "『閃け白刃（ホワイトブレイド）』　対象に【アタック：白Ａｓレベル《ＯＤ》＋１】を行い、その後【エクストルード】を行ってもよい。"], [2, "『二重に響け白刃（ホワイト・ダブルストライク）』　対象に【アタック：１】を行い、その後【アタック：１】を行う。"], [3, "『白き弾丸にて狙い撃て（ホワイト・スナイパー）』　対象に【アタック：２】を行う。対象が自身よりも低い「高度」に存在する場合、代わりに【アタック：４《ＯＤ》】を行う。"], [4, "『白き弾丸よ降り注げ（ホワイト・バレットシャワー）』　対象に【アタック：３《ＯＤ》】を行い、続けて【アタック：３】を行う。"], [5, "『白光の如く駆けよ（フラッシュ・ランニング）』　【ムーヴ：３】を行う。"], [6, "『閃光の突撃（フラッシュチャージ）』　対象が存在するセルへ移動する。"], [7, "『白光の衣（ホワイト・エンチャント）』　直後のアタックネイルで、現在の自身の「シールドレス」の枚数以上の成功数が出た場合、自身の「シールドレス」を１枚回復する。"], [8, "『荘厳なりし白の塔（ホワイトゴールドタワー）』　クリンナップフェイズ終了時まで、自身が「高度：８《ＯＤ》」に存在するものとして扱う。"], [9, "『輝きの盾（ホワイトシールド）』　自身のパッシヴドレスの「ブロックナンバー」に６を追加する。この効果はこのダイスロールの結果のみに有効である。"], [10, "『より疾きは光の一手（ライトニング・インターセプト）』　移動を行ったユニットに対し、即座に【アタック：２】を行う。"], [11, "『白の語り部（ホワイトテラー）』　ダイスロールで使用されたダイス全ての出目を６に変更する。"]];
            } else if ("D"['$===']($case)) {
              table = [[1, "『愚者の白（ホワイトフール）』"], [2, "『魔術師の白（ホワイトマジシャン）』　自身が「高度：６」以上に存在する時、ドロゥ数を＋１しても良い。"], [3, "『女教皇の白（ホワイトハイプリエステス）』　１ラウンドに１回まで、自身のダイスロールの出目ひとつを１増加しても良い。"], [4, "『女帝の白（ホワイトエンプレス）』　自身の白のアタックネイルに、以下の一文を追加する。「続けて【アタック：１】を行う。射程はこのアタックネイルに準ずる」"], [5, "『皇帝の白（ホワイトエンペラー）』　自身の「シールドレス」が破壊される度に、１ｄ６のダイスロールを行う。この時１か２の出目が出た場合「シールドレス」を１枚回復する。"]];
            } else if ("O"['$===']($case)) {
            ...
            if ("L"['$===']($case)) {
              table = [[1, "『碧空の剣（ストラトスフィア・ブレイド）』　対象に【アタック：３】を行う。"], [2, "『蒼天の剣靴（ストラトスフィア・ブレイドブーツ）』　対象に【アタック：１】を行う。対象が自身よりも低い「高度」に存在する場合、代わりに【アタック：４《ＯＤ》】を行い、対象の存在するセルへ移動する。"], [3, "『強襲翼撃（ウィング・ブレイド）』　対象に【アタック：３《ＯＤ》】を行い、対象の存在するセルへ移動する。"], [4, "『蒼弓の猛撃（ブルー・アローレイン）』　対象に【アタック：１】を行う。対象が自身よりも低い「高度」に存在する場合、続けて【アタック：３】を行う。"], [5, "『空歩き（エアステップ）』　【ムーヴ：２】を行う。このムーヴネイル以降、クリンナップフェイズ終了時まで、自身が「高度：６《ＯＤ》」に存在するものとして扱う。"], [6, "『凪歩き（カームステップ）』　【ムーヴ：１】を行う。移動先のセルにリヴラドールが存在する場合、【エクストルード】を行っても良い。"], [7, "『風の道標（ウィンドサインポスト）』　このアタックネイルの効果で「シールドレス」を１枚以上破壊した場合、アタックネイルの処理が終わった後、【ドロゥ：１】を行う。"], [8, "『精密思考（シャープセンス）』　このアタックネイルでは、攻撃対象のブロックナンバーのうち「３」を無視して攻撃を行うことが出来る。"], [9, "『思考の渦（ぐるぐる）』　ダイスロールに使用されたダイスひとつを指定する。そのダイスを振り直させる。"], [10, "『空翔けの回避（レビテート）』　そのアタックネイルのダイスロールで使用されたダイス全ての出目を１減少する。"], [11, "『たゆたう心、空の様に（ストラトスフィア・ハート）』　自身に適用された【エクストルード】を打ち消し、元のセルへと戻る。その後【ドロゥ：１】を行う。"]];
            } else if ("D"['$===']($case)) {
            ...
            if ("D"['$===']($case)) {
              table = [[1, "『黒の餓狼（ブラックソード・ウルフ）』　1.このユニットが使用するアタックネイルの対象を１増加しても良い。　2.このユニットの【アタック：Ｘ】で３以上の成功数が出た場合、自身のシールドレスを１枚回復する。"], [2, "『白の鋼鉄騎士（ぼくめつのりゅう）』　このユニットが使用するアタックネイルの対象を１増加しても良い。"], [3, "『赤の飛龍（クロムクラーク）』　1.このユニットが使用するアタックネイルの対象を１増加しても良い。　2.このユニットのアタックネイルの【アタック：Ｘ】のＸを２増加する。"], [4, "『青の翼龍（ヴァイエル）』　1.このユニットが使用するアタックネイルの対象を１増加しても良い。　2.このユニットが「高度：９」以上に存在する限り、ブロックナンバーに４を追加する。"], [5, "『緑の操り人形（グリーン・ジェイラー）』　自身を含む、このユニットと同じセルに存在するユニットは、クリンナッププロセスの終了時にシールドレスを１枚失う。"]];
            }
          }

          ;
          return self.$get_table_by_number(cardindex, table);
        }, $LiveraDoll_get_card_text$5.$$arity = 3), nil) && 'get_card_text';
...
//# sourceMappingURL=default~LiveraDoll~index.js.map