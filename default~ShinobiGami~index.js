(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["default~ShinobiGami~index"],{

/***/ "7kAL":
/*!*******************************************************************!*\
  !*** ./node_modules/bcdice/lib/bcdice/game_system/ShinobiGami.js ***!
  \*******************************************************************/
      ...
      (function ($base, $super, $parent_nesting) {
        ...
        Opal.def(self, '$result_2d6', $ShinobiGami_result_2d6$2 = function $$result_2d6(total, dice_total, _dice_list, cmp_op, target) {
          ...
          if ($truthy($rb_le(dice_total, 2))) {
            ...
            return $$($nesting, 'Result').$critical($$($nesting, 'SPECIAL'));
          } else if (target['$==']("?")) {
            return nil;
          ...
          ;
        }, $ShinobiGami_result_2d6$2.$$arity = 5);
        self.$register_prefix("\\d*SG");
        Opal.def(self, '$eval_game_system_specific_command', $ShinobiGami_eval_game_system_specific_command$3 = function $$eval_game_system_specific_command(command) {
          var self = this,
              result = nil,
              $ret_or_1 = nil,
              $ret_or_2 = nil;
          ...
          result = function () {
            if ($truthy($ret_or_1 = function () {
              if ($truthy($ret_or_2 = self.$action_roll(command))) {
                return $ret_or_2;
              } else {
          ...
          if (command['$==']("MT")) {
            return self.$sinobigami_metamorphose_table();
          }

          ;
          return nil;
        }, $ShinobiGami_eval_game_system_specific_command$3.$$arity = 1);
        Opal.const_set($nesting[0], 'RTT', $$$($$($nesting, 'DiceTable'), 'SaiFicSkillTable').$new([["器術", ["絡繰術", "火術", "水術", "針術", "仕込み", "衣装術", "縄術", "登術", "拷問術", "壊器術", "掘削術"]], ["体術", ["騎乗術", "砲術", "手裏剣術", "手練", "身体操術", "歩法", "走法", "飛術", "骨法術", "刀術", "怪力"]], ["忍術", ["生存術", "潜伏術", "遁走術", "盗聴術", "腹話術", "隠形術", "変装術", "香術", "分身の術", "隠蔽術", "第六感"]], ["謀術", ["医術", "毒術", "罠術", "調査術", "詐術", "対人術", "遊芸", "九ノ一の術", "傀儡の術", "流言の術", "経済力"]], ["戦術", ["兵糧術", "鳥獣術", "野戦術", "地の利", "意気", "用兵術", "記憶術", "見敵術", "暗号術", "伝達術", "人脈"]], ["妖術", ["異形化", "召喚術", "死霊術", "結界術", "封術", "言霊術", "幻術", "瞳術", "千里眼の術", "憑依術", "呪術"]]], $hash2(["s_format", "rtt_format"], {
          "s_format": "『%<category_name>s』%<skill_name>s",
          "rtt_format": "ランダム指定特技表(%<category_dice>d,%<row_dice>d) ＞ %<text>s"
        })));
        self.$private();
        Opal.def(self, '$action_roll', $ShinobiGami_action_roll$4 = function $$action_roll(command) {
          var self = this,
              parser = nil,
              cmd = nil,
              times = nil,
              ...
              $ret_or_4 = nil,
              dice_list_full = nil,
              dice_list_full_str = nil,
              dice_list = nil,
              dice_total = nil,
              total = nil,
              result = nil,
              sequence = nil;
          parser = $$$($$($nesting, 'Command'), 'Parser').$new(/\d*SG/, $hash2(["round_type"], {
          ...
          times = function () {
            if ($truthy(cmd.$command()['$=~'](/^\d/))) {
              return cmd.$command().$to_i();
            } else {
              return 2;
            }
          ...
          if ($truthy($rb_le(times, 1))) {
            return nil;
          }

          ;

          if ($truthy($ret_or_3 = cmd.$critical())) {
            $ret_or_3;
          } else {
            $writer = [12];
            $send(cmd, 'critical=', Opal.to_a($writer));
            $writer[$rb_minus($writer["length"], 1)];
          }

          ;

          if ($truthy($ret_or_4 = cmd.$fumble())) {
            $ret_or_4;
          } else {
            $writer = [2];
            $send(cmd, 'fumble=', Opal.to_a($writer));
            $writer[$rb_minus($writer["length"], 1)];
          }

          ;
          dice_list_full = self.randomizer.$roll_barabara(times, 6).$sort();

          if ($truthy($rb_gt(times, 2))) {
          ...
          result = function () {
            if ($truthy($rb_le(dice_total, cmd.$fumble()))) {
              return $$($nesting, 'Result').$fumble("ファンブル");
            } else if ($truthy($rb_ge(dice_total, cmd.$critical()))) {
              return $$($nesting, 'Result').$critical($$($nesting, 'SPECIAL'));
            } else if ($truthy(cmd.$cmp_op()['$nil?']())) {
              return $$($nesting, 'Result').$new();
            } else if ($truthy($rb_ge(total, cmd.$target_number()))) {
              return $$($nesting, 'Result').$success("成功");
            } else {
              return $$($nesting, 'Result').$failure("失敗");
            }
          ...
          sequence = ["" + "(" + cmd.$to_s("after_modify_number") + ")", dice_list_full_str, "" + dice_total + "[" + dice_list.$join(",") + "]" + $$($nesting, 'Format').$modifier(cmd.$modify_number()), total, result.$text()].$compact();
          $writer = [sequence.$join(" ＞ ")];
          $send(result, 'text=', Opal.to_a($writer));
          $writer[$rb_minus($writer["length"], 1)];
          ;
          return result;
        }, $ShinobiGami_action_roll$4.$$arity = 1);
        ...
        Opal.const_set($nesting[0], 'DEMON_SKILL_TABLES', [$hash2(["name", "page", "table"], {
          "name": "妖魔忍法表A",
          "page": "(怪p.252)",
          "table": ["【震々】", "【神隠】", "【夜雀】", "【猟犬】", "【逢魔時】", "【狂骨】"]
        }), $hash2(["name", "page", "table"], {
          "name": "妖魔忍法表B",
          "page": "(怪p.253)",
          "table": ["【野衾】", "【付喪神】", "【見越】", "【木魂】", "【鵺】", "【生剥】"]
        }), $hash2(["name", "page", "table"], {
          "name": "妖魔忍法表C",
          "page": "(怪p.254)",
          "table": ["【百眼】", "【呑口】", "【荒吐】", "【怨霊】", "【鬼火】", "【蛭子】"]
        })].$freeze());
        return self.$register_prefix("MT", $$($nesting, 'RTT').$prefixes(), $$($nesting, 'TABLES').$keys());
...
//# sourceMappingURL=default~ShinobiGami~index.js.map